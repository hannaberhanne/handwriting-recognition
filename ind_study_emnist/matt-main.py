...
def evaluate_model():
    ...
    #IMPORTANT: Looking at a Test Image    
    #Let's see what a test image looks like
    print(f"images: {images.shape}")
    the_image = images[1, 0, :, :]
    print(f"images: {the_image.shape} ")
    ...
    print("Result image saved as test_image.png")
...
def predict_custom_image(path):
    ...
    #IMPORTANT: Looking at the custom-predict image
    # We are viewing the custom-predict image exactly as it goes into the model
    the_result_image = tensor[0,0,:,:]
    print(the_result_image)
    print(f"Predicted Letter: {predicted_letter}")
    plt.imshow(the_result_image, cmap='gray')
